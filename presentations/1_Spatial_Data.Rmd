...
2. Learning about the `sp` library and how to create, manipulate, displaty, and otherwise use `Spatial*` objects to reprent spatially relevant data.
...
Since we use coordiantes all the time in our analyses, `gstudio` has included some helper functions as long as you are keeping our data in a `data.frame`.  In the *Arapatus attenuatus* data set there are locale coordinates defined as Longitude and Latitude.  By defult it produces just a `data.frame`.
...
You can see the tranformations in the coordinate system by comparing the plots below.  The relative position of each point is the same, it has just been loaded into a new space.
...
By default it makes the points filled and black.  Since this is a projected set of data, we should probably change the coordinate space so it knows that these are supposed to be mapped onto a projection in decimal degrees.  If you look at the figure above, you will see that the x- and y- limits are maximized in the graphic display, not scaled to be spatially coherient.  To change the coordinate system, we add a coordinate tranformation to the data.
...
1. The [WorldClim](http://worldclim.org) organization makes a set of bioclimatic data avaialbe for free.  Grab the raster layers for Tile 22 (they are in the <tt>LGDA-Data</tt> folder) and crop them down to an extent that covers the entirety of sampling locations for the *arapat* data set.

2. Using the population locations in the *arapat* dataset, extract the value for each layer.  To what extent are these variables correlated with eachother?  At what point will we need to be concerned if we are using them as input into statistical models?
