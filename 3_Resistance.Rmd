...
These layers are provided in the <tt>LGDA-Data/dogwood</tt> folder as *.asc raster files.  
...
require(raster)
pines <- raster("./LGDA-Data/dogwood/pines.asc")
...
```{r}
trees <- read.table("./LGDA-Data/dogwood/tree_coords.txt")
...
Since it takes a bit of time to extract the distance matrices from the rasters, we have provided some pre-configured distance matrices for the trees identified above.  These rasters are located in the <tt>LGDA-Data/dogwood</tt> folder.  For these rasters we estimate cost distance across a range of values representing both avoidance (10:1, 7:1, 5:1, and 2:1) and preference (1:2, 1:5, 1:7 1:10) of the feature.
...
```{r}
lcp.files <- list.files("./LGDA-Data/dogwood/", pattern="lcp*", full.names = TRUE)
library(vegan)
...
```{r}
load("LGDA-Data/dogwood/lcp.mh.7.1.rda")
MH <- D
...
```{r}
load("LGDA-Data/dogwood/lcp.cv.1.10.rda")
CV <- D
...
```{r}
load("LGDA-Data/dogwood/lcp.cv.10.1.rda")
X2 <- D
...
```{r}
load("LGDA-Data/dogwood/mixedhardwoods.rda")
mh <- mh * 6 + 1
