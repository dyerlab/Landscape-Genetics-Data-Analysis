...
Here is a raw AFLP file

```{r}
path <- system.file("extdata", "data_aflp.csv", package = "gstudio")
system( paste("head",path), intern = FALSE )
```
...
Here is a two-column microsat locus
...
Mapping locales is also a common enough process that *gstudio* provides helper 
...
p <- ggmap( map )
p <- p + geom_strata(aes(x=Longitude, y=Latitude, stratum=Population), data=arapat)
p
```
