...
1. The [WorldClim](http://worldclim.org) organization makes a set of bioclimatic data avaialbe for free.  Grab the raster layers for Tile 22 and extract the value for each layer recorded at each of the sampling sites.  To what extent are these variables correlated with eachother?
...
2. Using the methods outlined in the Cropping via Convex Hull approach, crop all the bio rasters to an are surrounding the sample locations in Baja California. We will be using these rasters later in the week.
