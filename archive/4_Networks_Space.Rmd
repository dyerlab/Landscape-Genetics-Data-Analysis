...
![Graph overlain onto map object](./media/rodney_graph.png)
...
Additional data could be extracted from other rasters.  See http://worldclim.org for some example data that may prove useful.  Here I'm using the namespace designator (`raster::`) in front of the `extract()` function because of something called namespace collisions.  What this means is that I loaded in the `raster()` package way up in the code, so all the functions in that package are loaded into memory.  In the interim, I've also loaded in other packages, some of whih also provide a function called `extract()`.  What happens in these cases is that the package loaded last 'masks' the functions from the previously loaded packages.  So, when I'm compiling this markdown as a book, when I hit the above chunk, I get the following error.

![Namespace collision in cryptic 'R programmer' (e.g., do not care much for the sanity of the R user) speak.](./media/namespace_collision.png)

This message says that I'm calling a function `extract` on an object that is some kind of `Raster` but the currently loaded `extract()` function does not know how to deal with rasters...  Odd, because we *know* there is an `extract()` function that works on Rasters (see first Chapter).  One of the strategies I use when encountering cryptic messages like this is to look to see how many functions there are named `extract()`.  You can do this by typing in `?extract` in the command line.  When working on this document, my computer has at least the following three

![The `extract` functions in my global namespace.](./media/extract_help.png)


So, this tells me that there are at least 3 different `extract()` functions in effect!  To get around this, I can tell R to use the extract function that is from the raster library using the notation as above (e.g., library name + double colons + function name) and it will *do the right thing.*  Sometimes you need to hack your way around these kinds of issues.  If you become a writer of packages, you could also not be a jerk and come up with better names that don't obliterate functions from other packages that people may be working with at the same time...  Just saying.
...
```{r warning=FALSE,message=FALSE}
elev <- raster::extract( alt, edge )[[1]]
...
```{r echo=TRUE, eval=FALSE}
edge_values <- raster::extract( alt, all_edges, fun=max, na.rm=TRUE, df=TRUE)
```
